...
exports.dpInitCustomSelect = dpInitCustomSelect;
function handleContainerClick(e) {
  e.stopPropagation();
  this.classList.toggle("open");

  // Dynamically find the latest select-value and select-items within this this
  var updatedSelectedItemDiv = this.querySelector(".select-value");
  var optionsList = this.querySelector(".select-items");
...
function dpInitCustomSelect() {
  /* Remove any existing custom selects to prevent duplicates */
  document.querySelectorAll(".select-value, .select-items").forEach(function (el) {
    return el.remove();
  });
  document.querySelectorAll(".select-container").forEach(function (el) {
    return el.classList.remove("open");
  });
  var customSelectElements = document.getElementsByClassName("select-container");
  var _loop = function _loop() {
    var _selectElement$option;
    var container = customSelectElements[i];
    var innerSelect = container.querySelector(".custom-select");
    var selectElement = container.getElementsByTagName("select")[0];
    ...
    selectedItemDiv.innerHTML = ((_selectElement$option = selectElement.options[0]) === null || _selectElement$option === void 0 ? void 0 : _selectElement$option.innerHTML) || "";
    ...
    var _loop2 = function _loop2(j) {
      ...
      optionDiv.addEventListener("click", function () {
        ...
        document.querySelectorAll(".select-option span:last-child").forEach(function (icon) {
          return icon.innerHTML = svgUnchecked;
        });
...
document.addEventListener("click", function () {
  document.querySelectorAll(".select-items").forEach(function (el) {
    return el.classList.add("select-hide");
  });
  document.querySelectorAll(".select-container").forEach(function (el) {
    return el.classList.remove("open");
  });
...
function closeAllSelect(element) {
  ...
  for (i = 0; i < optionsListsLength; i++) {
    if (selectedIndexesToExclude.indexOf(i) === -1) {
      ...
      optionsLists[i].parentElement.parentElement.parentElement.classList.remove("open");
    }
  }
}
